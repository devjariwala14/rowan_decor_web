...
if (isset($_COOKIE['edit_id'])) {
	...
	$stmt = $obj->con1->prepare("SELECT * FROM `objects` WHERE id=?");
...
if(isset($_REQUEST['btnsubmit']))
{
	...
	try
	{
		$stmt = $obj->con1->prepare("INSERT INTO `objects`(`object_name`,`status`) VALUES (?,?)");
		$stmt->bind_param("ss",$object_name,$status);
		$Resp=$stmt->execute();
		if(!$Resp)
		{
...
if(isset($_REQUEST['btnupdate']))
{
	...
	try
	{
		...
		$stmt = $obj->con1->prepare("UPDATE `objects` SET `object_name`=?,`status`=? where id=?");
		$stmt->bind_param("ssi",$object_name,$status,$e_id);
		$Resp=$stmt->execute();
		if(!$Resp)
		{
    ...
    <div class="col-xl">
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h5 class="mb-0"> <?php echo (isset($mode)) ? (($mode == 'view') ? 'View' : 'Edit') : 'Add' ?> Object
                </h5>
            ...
            <div class="card-body">
                <form method="post">
                    <div class="col mb-3">
                        <label class="form-label" for="basic-default-fullname">Object Name</label>
                        <input type="text" class="form-control" name="object_name" id="object_name"
                            value="<?php echo (isset($mode)) ? $data['object_name'] : '' ?>"
